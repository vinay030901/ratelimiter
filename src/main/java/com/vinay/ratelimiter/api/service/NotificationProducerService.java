...
public class NotificationProducerService {
        ...
        public Map<String, Object> handleNotification(NotificationRequest request, String idempotencyKey) {
                ...
                NotificationRequest enrichedRequest = new NotificationRequest(reqId, request.userId(),
                                request.channel(), request.destination(), request.subject(), request.message());
                ...
                var config = rateLimitProperties.getChannels()
                                .getOrDefault(enrichedRequest.channel().toUpperCase(),
                                                new NotificationRateLimitProperties.LimitConfig());
                ...
                // enqueue in Kafka
                log.debug("Sending notification to Kafka topic: notifications.requests, requestId: {}", reqId);
                var future = kafkaTemplate.send("notifications.requests", reqId, enrichedRequest);
                future.thenAccept(result -> log.debug("Kafka send success, topic-partition: {}",
                                result.getRecordMetadata()))
                                .exceptionally(ex -> {
                                        log.error("Kafka send failed for requestId {}: {}", reqId, ex.getMessage());
                                        return null;
                                });
