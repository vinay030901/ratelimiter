...
public class NotificationConsumer {
    ...
    public NotificationConsumer(NotificationDispatcher dispatcher, NotificationRepository repository, NotificationMetrics notificationMetrics) {
    ...
    public void consumeDlt(NotificationRequest request, @Header(name = KafkaHeaders.DLT_EXCEPTION_MESSAGE,required = false) String exceptionMessage, @Header(name = KafkaHeaders.DLT_EXCEPTION_STACKTRACE,required = false) byte[] exceptionStack) {
        ...
        log.error("Permanently failed notification {} due to error: {}, stacktrace: {}", request.requestId(), msg, stack);
        repository.findByRequestId(request.requestId()).ifPresent(entity -> {
